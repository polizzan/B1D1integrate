  ...
  map_df(select.countries, function(x){
    ...
    CNTRY.fx <-
      HMDHFDplus::readHFD(unz(description = here::here("data", "asfr.zip"), 
                              filename = "Files/zip_w/asfrVH.txt")) %>% 
      filter(Code == code) %>% 
...
label.df <-
  data.frame(
    ...
    y = rep(Inf, 9)
               ...
               Framework = factor(Framework, levels = c("B1D1", "Traditional", "Ratio")))
...
for(x in select.countries){
  ...
  fig.1 <-
    ...
    geom_label(data = label.df, 
               ...
               inherit.aes = FALSE, parse = TRUE, size = 3.5, label.size = NA, fill = NA) +
...
tempo.plot <-   
  ...
  geom_text(data = 
            ...
            aes(x = Cohort, y = tempo), 
            label = select.country, hjust = 0, vjust = 2, inherit.aes = FALSE) +
  scale_x_continuous(breaks = seq(cohort.min, cohort.max, 10)) +
...
## get legend
g <- cowplot::get_plot_component(la.plot +
                                   theme(legend.title = element_text(size = 14),
                                         legend.key.width = unit(0.905, "in")), 
                                 "guide-box", 
                                 return_all = TRUE)
...
fig.2 <-
  cowplot::ggdraw(OMEGA.plot +
                    geom_text(data = 
                                ...
                                filter(Country == select.country) %>% 
                                filter(Cohort == min(Cohort)), 
                              aes(x = Cohort, y = OMEGA), 
                              label = select.country, hjust = -0.5, vjust = 0.5, inherit.aes = FALSE) +
                    geom_line(linewidth = 1, show.legend = FALSE) + 
  ...
  cowplot::draw_plot(la.plot + 
                     ...
                     x = 0.55, y = 0.2,
                     height = 0.35, width = 0.35) +
  cowplot::draw_plot(g[[3]]$grobs[[1]], 0.02, -0.84)
